...
static void doubleArray(PvaClientPtr const &pva,string const & channelName,string const & providerName )
{
    ...
    value = pva->channel(channelName)->get()->getData()->getDoubleArray();
    cout << "get doubleArray with defaults " << value << endl;
    // following is like the previous but without defaults
    value = pva->channel(channelName,providerName,5.0)
         ->get("field(value,alarm,timeStamp)")->getData()->getDoubleArray();
    cout << "get doubleArray without defaults " << value << endl;

    data = shared_vector<double>(num);
    ...
    cout << "get doubleArray long way" << value << endl;
}
...
int main(int argc,char *argv[])
{
    ...
    for(size_t i=0; i<channelNames.size(); ++i) {
        try {
            doubleArray(pva,channelNames[i],providerName);
